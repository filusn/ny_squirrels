...
st.markdown(f'Short analysis and visualisation of [Central Park squirrels data]{ny_opendata} made to try out the Streamlit.')
...
"""
st.write('In our data Central Park is divided into hectare grid and numbered from \'01A\' to \'42I\' with numerical axis running north-to-south and alphabetical running east-to-west. Squirrels sightings from dataset take place only on 339 of 378 (350 \'countable\') of these hectares.')
...
st.write(f'Most squirrels were seen on hectare number {max_squirrels_hec} - {max_squirrels_hec_nr}. Least squirrels were on hectare {min_squirrels_hec} - {min_squirrels_hec_nr}.')


"""
### Time of squirrels sightings
"""
st.write(f'Squirrels were seen more in the afternoons - {len(data.loc[data["Shift"] == "PM"])} times. In the morning shifts researchers have met the squirrels {len(data.loc[data["Shift"] == "AM"])} times.')
...
primary_color_chart = alt.Chart(data).mark_bar().encode(x=alt.X('Primary Fur Color'), y='count()').properties(height=300, width=300)
high_color_chart = alt.Chart(data).mark_bar().encode(x=alt.X('Highlight Fur Color'), y='count()').properties(height=300, width=300)
...
"""
# All activity
running = data['Running'].sum()
chasing = data['Chasing'].sum()
climbing = data['Climbing'].sum()
eating = data['Eating'].sum()
foraging = data['Foraging'].sum()
activity_data = pd.DataFrame([{'Activity': 'running', 'Count': running}, {'Activity': 'chasing', 'Count': chasing}, {'Activity': 'climbing', 'Count': climbing}, {'Activity': 'eating', 'Count': eating}, {'Activity': 'foraging', 'Count': foraging}])

activity_chart = alt.Chart(activity_data).mark_bar().encode(x='Activity:O', y='Count:Q')

# Activity by shift
activity_by_shift = data.groupby('Shift')[['Running', 'Chasing', 'Climbing', 'Eating', 'Foraging']].sum()
activity_by_shift = activity_by_shift.reset_index()
activity_by_shift = activity_by_shift.melt('Shift', var_name='Activity', value_name='Count')

activity_shifts_chart = alt.Chart(activity_by_shift).mark_bar().encode(x=alt.X('Activity:N'), y='Count', column='Shift:N')
...
foraging_data.columns = ['Age1', 'Foraging Counts']
age_counts = age_counts.append(age_counts, ignore_index=True).sort_values(by='index')
fora = foraging_data['Foraging Counts'] / age_counts['Age']
# <Some of my value/column names are really misleading here>
fora
st.write(foraging_data)
st.write(age_counts)
foraging_age_chart = alt.Chart(foraging_data).mark_bar().encode(x=alt.X('percentage(Foraging):Q', stack='normalize', axis=alt.Axis(format='%')), y='Age', color='Foraging').properties(width=300, height=300)
...
date_selectbox = st.sidebar.selectbox('Date', ['All dates'] + sorted(data['Normal Date'].unique()))
...
checked_ages = [age for age, is_checked in dict(Adult=age_checkbox_adult, Juvenile=age_checkbox_juvenile, Unknown=age_checkbox_unknown).items() if is_checked]
...
checked_colors = [color for color, is_checked in dict(Black=color_black, Cinnamon=color_cin, Gray=color_gray, Unknown=color_unknown).items() if is_checked]
...
# DATA FILTERING
map_data = data.loc[data['Normal Date'] == date_selectbox] if date_selectbox != 'All dates' else data
...
squirrels_number_slider = st.sidebar.slider('Number of squirrels', min_value=0, max_value=len(map_data), value=default_value)
